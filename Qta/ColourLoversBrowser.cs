...
{
	/// <summary>
	/// Description of ColourLoversBrowser.
	/// </summary>
	public partial class ColourLoversBrowser : Form
	{
		...
		List<ColourLoversPalette> palettes;
		
		public ColourLoversBrowser(PaletteEditor pe)
		{
			InitializeComponent();
			this.pe = pe;
			setCurrentRequest(null);
			palettes = new List<ColourLoversPalette>();
		}
		...
		void parsePalettesXml(XPathDocument doc)
		{
			...
			while(paletteNodes.MoveNext()){
				ColourLoversPalette pal = new ColourLoversPalette();
		...
		void ResultListBoxDrawItem(object sender, DrawItemEventArgs e)
		{
			...
			e.DrawBackground();
			if(e.Index<0) return;
			ColourLoversPalette pal = (ColourLoversPalette)lb.Items[e.Index];
		...
		ColourLoversPalette GetCurrentlySelectedPalette()
		{
			if(resultListBox.SelectedIndex<0) return null;
			return (ColourLoversPalette)resultListBox.SelectedItem;
		}
		
		void VisitPaletteURLToolStripMenuItemClick(object sender, EventArgs e)
		{
			ColourLoversPalette pal = GetCurrentlySelectedPalette();
		...
		void ApplyToolStripMenuItemClick(object sender, EventArgs e)
		{
			...
			if(pal==null) return;
			if(pe==null) {
				MessageBox.Show("This browser can't apply palettes, sorry.");
				return;
			}
			pe.Pal.setKeyPointsByColors(pal.Colors);
			pe.UpdateView();
		}
